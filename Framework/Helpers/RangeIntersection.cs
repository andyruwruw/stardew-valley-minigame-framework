...
using MinigameFramework.DataStructures.Primitives;
...
namespace MinigameFramework.Helpers
{
    ...
    internal static class RangeIntersection
    {
        ...
        {
            bool isRectangle = shape1 is DataStructures.Primitives.Rectangle || shape2 is DataStructures.Primitives.Rectangle;
            bool isCircle = shape1 is Circle || shape2 is Circle;
            bool isLine = shape1 is Line || shape2 is Line;

            if (isRectangle && !isCircle && !isLine)
            {
                return RangeIntersection.IsIntersectingRectangles(
                    (DataStructures.Primitives.Rectangle)shape1,
                    (DataStructures.Primitives.Rectangle)shape2
                );
            }
            if (isCircle && !isRectangle && !isLine)
            {
                return RangeIntersection.IsIntersectingCircles(
                    (Circle)shape1,
                    (Circle)shape2
                );
            }
            ...
            {
                return RangeIntersection.IsIntersectingLines(
                    (Line)shape1,
                    (Line)shape2
                );
            }
            if (isCircle && isRectangle && !isLine)
            {
                return RangeIntersection.IsIntersectingCircleAndRectangle(
                    (Circle)(shape1 is Circle ? shape1 : shape2),
                    (DataStructures.Primitives.Rectangle)(shape1 is DataStructures.Primitives.Rectangle ? shape1 : shape2)
                );
            }
        ...
        private static bool IsIntersectingRectangles(
            DataStructures.Primitives.Rectangle rectangle1,
            DataStructures.Primitives.Rectangle rectangle2
        )
        {
        ...
        private static bool IsIntersectingCircleAndRectangle(
            Circle circle,
            DataStructures.Primitives.Rectangle rectangle
        )
        {
            float Xn = Math.Max(
                rectangle.GetNorthWestCorner().X,
                Math.Min(circle.GetCenter().X, rectangle.GetSouthEastCorner().X)
            );
            float Yn = Math.Max(
        ...
        {
            bool startInCircle = Distance.Pythagorean(
                line.GetStart(),
                circle.GetCenter()
            ) <= circle.GetRadius();
            bool endInCircle = Distance.Pythagorean(
                line.GetEnd(),
                circle.GetCenter()
            ) <= circle.GetRadius();
